...
def extract_data():
    ...
    log.info("[START] Extraindo dados de Eventos do flakeflow")
...
def load_table():
    ...
    with ddb.connect() as con:
        ...
        con.execute("CALL postgres_execute('pg', 'TRUNCATE TABLE ferramental.eventos')")

        total_rows = (
            con.execute(
                f"""
            INSERT INTO pg.ferramental.eventos
    ...
    sentinel.touch()
    log.info(f"[DONE] ferramental.eventos recarregada — {total_rows} registros")
...
@dag(
    dag_id="BRONZE_F_EVENTOS",
    default_args=default_args,
    schedule="0 7 * * *",
    catchup=False,
    max_active_runs=1,
    tags=["flakeflow", "eventos", "bronze"],
)
def dag_factory():
